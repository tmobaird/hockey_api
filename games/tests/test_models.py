...
class GameTestCase(TestCase):
    ...
    def test_clean_fields_fails_when_period_is_not_allowed_type(self):
        with self.assertRaises(ValidationError):
            game = Game(start='10:00.000', period='BAD', home_team=self.home_team, away_team=self.away_team)
            game.clean_fields()

    def test_clean_fields_does_not_raise_when_period_is_allowed_type(self):
        try:
            game = Game(start='10:00.000', period='1', home_team=self.home_team, away_team=self.away_team)
            game.clean_fields()
